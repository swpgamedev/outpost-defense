...
func draw_line_relative_thick(point_a: Vector3, point_b: Vector3, thickness: float = 2, color: Color = Color.RED):
	...
	if draw_debug.mesh is ImmediateMesh:
		...
		var verticies_stirp_order = [4, 5, 0, 1, 2, 5, 6, 4, 7, 0, 3, 2, 7, 6]
		var local_B = (point_b - point_a)
		
		for v in range(14):
			var vertex = normal if \
				verticies_stirp_order[v] < 4 else \
				normal + local_B
			var final_vert = vertex.rotated(dir,
				PI * (0.5 * (verticies_stirp_order[v] % 4 + 0.25)))
			final_vert += point_a
			draw_debug.mesh.surface_add_vertex(final_vert)
		draw_debug.mesh.surface_end()
		

func draw_line_relative_pointy(point_a: Vector3, point_b: Vector3, thickness: float = 2, color: Color = Color.RED):
	point_b = point_a + point_b
